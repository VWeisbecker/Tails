...
ratios1=as.data.frame(cbind(ratios, rep(1:3,each=length(ratio_Tl))))
colnames(ratios1)=c("ratios","Factor")
as.factor(ratios1$Factor)

#run nonparametric comparison of means
kruskal.test(ratios~as.factor(Factor), data=ratios1 )

#since this is significant, run pairwise wilcoxon test with bonferroni holmes adjustment for multiple comparisons; this is a paired test so it explicitly compares data points from within each species
pairwise.wilcox.test(ratios1$ratios, ratios1$Factor, paired=TRUE, p.adjust.method = "BH")
