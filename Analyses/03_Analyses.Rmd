...
plot(data$Wt_Av_M,data$Tl_Av_M, col="red", pch=19, xlab="log Weigth (g)", ylab="log Tail length (mm)");
...
#To understand the mode of evolution (Brownian, Ornstein-Uhlenbeck, or Early burst), we run the fit.continuous function from the phylo package. Note that I have not been able to put this into a loop because of a warning message regarding the OU model for tail length.
...
```{r}
      ...
      ##dropping the interaction; the Pagel model does not work here and throws an error message, but Pagel models are nearly never a good fit for the data so we here proceed with the Brownian model that best fits the model with interactions. 
...
plot(data$Tl_mm~data$Bl_mm, col=colsLoco[data$Locomotor_use], pch = pchs[data$Clade], xlab="log Body length (mm)", ylab="log Tail length",asp=1 ); 
legend("bottomright",inset=0.2,levels(data$Locomotor_use) , pch=19, col=colsLoco, cex=cexes, x.intersp = xinter, y.intersp = yInter+0.2, text.width = textwidth, bty="n" );  
...
plot(data$Tl_mm~data$Wt_g, col=colsLoco[data$Locomotor_use], pch = pchs[data$Clade], xlab="log Weight (g)", ylab="log Tail length" , asp=1);
legend("bottomright",inset=0.25,levels(data$Clade) , pch=pchs,  cex=cexes,x.intersp = xinter, y.intersp = yInter, text.width = textwidth, bty="n" );
...
plot(data_ranges_Bl$Tl_mm~data_ranges_Bl$Bl_mm, col=colsLoco[data$Locomotor_use], pch = pchs[data$Clade], xlab="log Body length (mm)", ylab="log Tail length" , xlim=c(1.6,3.5), ylim=c(1.5,3.3) , asp=1); 
...
legend("bottomright",inset=0.25,levels(data$Locomotor_use) , pch=19, col=colsLoco, cex=cexes, x.intersp = xinter, y.intersp = yInter+0.1, text.width = textwidth, bty="n");  
...
plot(data_ranges_Wt$Tl_mm~data_ranges_Wt$Wt_g, col=colsLoco[data$Locomotor_use], pch = pchs[data$Clade], xlab="log Weigth (g)", ylab="log Tail length" , xlim=c(0.3,5.3), ylim=c(1.2,3.3), asp=1 ); 
...
legend("bottomright",inset=0.13,levels(data$Clade) , pch=pchs,  cex=cexes,x.intersp = xinter, y.intersp = yInter+0.3, text.width = textwidth, bty="n" );
