...
#In the plot, Peremelemorphs and the genus _Macropus_ are suggestive of different intercept and slope, respectively. Checking if the significant interaction is driven by these two
```{r}
...
#Is scaling in this group isometric?
ArbPentScanScaling=find.best.model(Tl_mm~Bl_mm,ArbPentScanTree,ArbPentScan);ArbPentScanScaling$AIC_Weights
summary(ArbPentScanScaling$Models$Brownian, type="marginal")

#The inspiration for this code comes from a researchgate entry by Pedro Jose Aphalo
slope <-ArbPentScanScaling$Model_summaries$Brownian$coefficients[2]
#I found the use of this in http://esapubs.org/archive/ecol/e088/020/pgls.R; this returns the standard error for the scaling coefficient
se <- as.numeric(sqrt(diag(ArbPentScanScaling$Models$Brownian$varBeta))[2])
df <- ArbPentScanScaling$Models$Grafen$dims$N
t_value_one <- (slope-1)/se
dt(t_value_one, df=df) / 2 # two sided test
...
ScanSaltScaling <- find.best.model(Tl_mm~Bl_mm,ScanSaltTree,ScanSalt);ScanSaltModel$AIC_Weights

slope <-ScanSaltScaling$Model_summaries$Brownian$coefficients[2]
se <- as.numeric(sqrt(diag(ScanSaltScaling$Models$Brownian$varBeta))[2])
df <- ScanSaltScaling$Models$Grafen$dims$N
t_value_one <- (slope-1)/se
dt(t_value_one, df=df) / 2 
...
TerrSaltScaling=find.best.model(Tl_mm~Bl_mm,TerrSaltTree,TerrSalt);TerrSaltModel$AIC_Weights

slope <-TerrSaltScaling$Model_summaries$Brownian$coefficients[2]
se <- as.numeric(sqrt(diag(TerrSaltScaling$Models$Brownian$varBeta))[2])
df <- TerrSaltScaling$Models$Grafen$dims$N
t_value_one <- (slope-1)/se
dt(t_value_one, df=df) / 2 
...
ArbSaltScaling=find.best.model(Tl_mm~Bl_mm,ArbSaltTree,ArbSalt);ArbSaltModel$AIC_Weights

slope <-ArbSaltScaling$Model_summaries$Brownian$coefficients[2]
se <- as.numeric(sqrt(diag(ArbSaltScaling$Models$Brownian$varBeta))[2])
df <- ArbSaltScaling$Models$Grafen$dims$N
t_value_one <- (slope-1)/se
dt(t_value_one, df=df) / 2 
...
plot(data$Tl_mm~data$Bl_mm, col=colsLoco[data$Locomotor_use], pch = pchs[data$Clade], xlab="log Body length (mm)", ylab="log Tail length",asp=1 ,bty="L");
abline(lm (Terr$Tl_mm~Terr$Bl_mm), col=colsLoco[5]);lines(x=c(1.7,3.1),y=c(1.66433210725893,2.63480806919339), col=colsLoco[5], lwd=2);
abline(lm (Scan$Tl_mm~Scan$Bl_mm), col=colsLoco[4]);lines(x=c(1.9,2.7),y=c(1.9958300045442,2.62692913039759), col=colsLoco[4], lwd=2)
abline(lm (Salt$Tl_mm~Salt$Bl_mm), col=colsLoco[3]);lines(x=c(2.3,2.83),y=c(2.10594027781454,2.78581969836552), col=colsLoco[3], lwd=2)
abline(lm (Pent$Tl_mm~Pent$Bl_mm), col=colsLoco[2]);lines(x=c(2.6,3.2),y=c(2.60573488742311,2.93380010371888),col=colsLoco[2], lwd=2)
abline(lm (Arb$Tl_mm~Arb$Bl_mm), col=colsLoco[1]);lines(x=c(1.8,2.7),y=c(1.8776347281739,2.67921326346783),col=colsLoco[1], lwd=2)
...
plot(tree, type="phylogram", cex=0.8, tip.color = colsLoco[data$Locomotor_use])
