...
plot(data$Wt_Av_M,data$Tl_Av_M, col="red", pch=19);points(data$Wt_Av_F,data$Tl_Av_F, col="blue", pch=19);points(data$Wt_g,data$Tl_mm, col="cyan", pch=19);abline(lm(data$Tl_Av_M~data$Wt_Av_M), col="red");abline(lm(data$Tl_Av_F~data$Wt_Av_F), col="blue"); abline(lm(data$Tl_mm~data$Wt_g), col="cyan")  
...
#The below gives a warning that the phylgenetic correlation matrix is singular when the untransformed tree is run. But there are no differences in significance levels regardless which one is used.
...
Arb_Tree=drop.tip(tree, name.check(tree,Arb_Weight)$tree_not_data )
...
#Model
TWNA=find.best.model(Tl_mm~Wt_g+Locomotor_use,NonArbTree,NonArb);TBNAP$AIC_Weights
...
plot(data$Tl_mm~data$Bl_mm, col=colsLoco[data$Locomotor_use], pch = pchs[data$Order], xlab="log Body length (mm)", ylab="log Tail length" ); legend("bottomright",inset=0.08,levels(data$Locomotor_use) , pch=19, col=colsLoco, cex=cexes, x.intersp = xinter, y.intersp = yInter, text.width = textwidth, bty="n");  lines(x=c(1.8,2.8),y=c(1.8776347281739,2.76827754516716), col=colsLoco[1], lwd=2); lines(x=c(2.6,3.2),y=c(2.60573488742311,2.93380010371888), col=colsLoco[2], lwd=2);
