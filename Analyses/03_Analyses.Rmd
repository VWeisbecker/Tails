...
plot(mean_range_data$Bl_mm~mean_range_data$range_mid_Bl, pch=19)
plot(mean_range_data$Tl_mm~mean_range_data$range_mid_Tl, pch=19)
plot(mean_range_data$Wt_g~mean_range_data$range_mid_Wt, pch=19)


#mean percentage of weight differences between known mean an inferred midpoing mean 
mean(abs( mean_range_data$Bl_mm-mean_range_data$range_mid_Bl)*100/mean_range_data$Bl_mm, na.rm = TRUE)
mean (abs( mean_range_data$Tl_mm-mean_range_data$range_mid_Tl)*100/mean_range_data$Tl_mm, na.rm = TRUE)

#Check why weight is so much larger - there's got to have been a mistake

mean(abs( mean_range_data$Wt_g-mean_range_data$range_mid_Wt)*100/mean_range_data$Wt_g, na.rm = TRUE)

cbind(mean_range_data$Wt_g,mean_range_data$range_mid_Wt, abs( mean_range_data$Wt_g-mean_range_data$range_mid_Wt)*100/mean_range_data$Wt_g)
...
#In the plot, Peremelemorphs and the genus _Macropus_ are suggestive of different intercept and slope, respectively. Thus checking if the significant interaction is driven by these two
```{r}
