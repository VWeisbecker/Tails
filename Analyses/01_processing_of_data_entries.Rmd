...
#Something like this - keep going here to weed out

data_for_mid_checks<-data_for_mid_checks(-c
...
#now re-process data_for_mid_checks to fill NAs in the species means, first by averaging male/female values, second (if sex averages are not available) by averaging sex-less min/max values. This overwrites the first pass summary version of data_for_mid_checks with the second pass summary version

for (i in 1:(nrow(data_for_mid_checks))){
  
   
      #if there is a male/female average value, use its average as final value (assuming that female ranges also mean that there is a male range)
      if(is.na(data_for_mid_checks$Wt_g[i])==TRUE & is.na(data_for_mid_checks$Wt_Av_F[i])==FALSE){
  
      data_for_mid_checks$Wt_g[i]<-mean(c(data_for_mid_checks$Wt_Av_F[i], data_for_mid_checks$Wt_Av_M[i]))
      }
  
      if(is.na(data_for_mid_checks$Bl_mm[i])==TRUE & is.na(data_for_mid_checks$Bl_Av_F[i])==FALSE){
  
      data_for_mid_checks$Bl_mm[i]<-mean(c(data_for_mid_checks$Bl_Av_F[i], data_for_mid_checks$Bl_Av_M[i]))
      }
    
      if(is.na(data_for_mid_checks$Tl_mm[i])==TRUE & is.na(data_for_mid_checks$Tl_Av_F[i])==FALSE){
  
      data_for_mid_checks$Tl_mm[i]<-mean(c(data_for_mid_checks$Tl_Av_F[i], data_for_mid_checks$Tl_Av_M[i]))
      }
  
  
      #if there are no male/female average values (regardless of newly created above or originally in the book; i.e. the male and female), average the ranges
      if(is.na(data_for_mid_checks$Wt_g[i])==TRUE & is.na(data_for_mid_checks$Wt_Av_F[i])==TRUE){
  
      data_for_mid_checks$Wt_g[i]<-mean(c(data_for_mid_checks$Wt_max[i], data_for_mid_checks$Wt_min[i]))
      }
  
      if(is.na(data_for_mid_checks$Bl_mm[i])==TRUE & is.na(data_for_mid_checks$Bl_Av_F[i])==TRUE){
  
      data_for_mid_checks$Bl_mm[i]<-mean(c(data_for_mid_checks$Bl_max[i], data_for_mid_checks$Bl_min[i]))
      }
    
      if(is.na(data_for_mid_checks$Tl_mm[i])==TRUE & is.na(data_for_mid_checks$Tl_Av_F[i])==TRUE){
  
      data_for_mid_checks$Tl_mm[i]<-mean(c(data_for_mid_checks$Tl_max[i], data_for_mid_checks$Tl_min[i]))
      }  
}
...
#Write the csv; row.names need to be FALSE because otherwise it saves with an extra column of numbers
write.csv(data_for_mid_checks, file="../Data/Full_dataset.csv", row.names=FALSE)
