...
numcols=as.data.frame(Data[,2:34])
...
#exclude wombats who are outliers
...
#deleting all entries that have NAs in their main data and locomotor use, assuming if they have Tl they also have the others
mars=mars[-which(is.na( mars$Tl_mm) & is.na(mars$Locomotor_use)),]

rod=rod[-which(is.na(rod$Tl_mm) & is.na(rod$Locomotor_use)),]
...
#Dasyuromorphs
dasy_data=mars[which(mars_synch$Order =="Dasyuromorph"),]
...
#Processing data for partitions with m/f data available (assuming if m present, f is also present)
```{r}
