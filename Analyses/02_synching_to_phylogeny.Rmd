...
#exclude wombats,koalas and marsupial moles who are outliers using grep (which returns the number of the row that has the species name). These are interesting and need to be discussed, but result in low model fits so that they are taken out
...
data$Order <- droplevels(data$Order, exclude= "Vombatiform")
...
#make sure the species names are in teh reows
...
save(dataFull,data,tree, dasy_data, dasy_tree,peram_data, peram_tree,dip_data,dip_tree, mf_data,mf_datatree, file = "../Data/Processed_Data_for_Pgls.rda")
